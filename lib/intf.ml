...
let read_intf dev net eth arp recv_push =
  ...
  | Error e ->
      ...
      >>= fun () ->
      Netif.disconnect net >>= fun () -> Lwt.return @@ recv_push None
  ...
  | Some ipv4_pkt ->
      ...
      if (not (Ipaddr.V4.Prefix.mem dst t.network)) && t.gateway = None then
        ...
        let query_ip =
          ...
          else match t.gateway with None -> assert false | Some gw -> gw
        in
        ...
        >>= (function
              ...
              | Error e ->
                  Log.err (fun m ->
                      m "%s Arpv4.query: %a" t.dev Arpv4.pp_error e))
        >>= fun () -> write_intf t eth arp send_st
  | None ->
      Log.warn (fun m -> m "%s send stream is closed!" t.dev)
...
let start_intf t net eth arp recv_push send_st () =
  Lwt.catch
    (fun () ->
      ...
      with e -> Lwt.fail e)
    (fun e ->
      Log.err (fun m -> m "%s on_intf: %s" t.dev (Printexc.to_string e)))
