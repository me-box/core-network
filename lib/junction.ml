...
module Local = struct
      ...
      Lwt.catch
        (fun () ->
          ...
          try
            ...
            let name = List.assoc "name" dict |> get_string in
            let ip =
              List.assoc "ip" dict |> get_string
              |> Ipaddr.V4.Prefix.of_address_string_exn |> snd
            in
      ...
      Lwt.catch
        (fun () ->
          ...
          try
            ...
            let name = List.assoc "name" dict |> get_string in
            let old_ip =
              List.assoc "old_ip" dict |> get_string |> Ipaddr.V4.of_string_exn
            in
            let new_ip =
              List.assoc "new_ip" dict |> get_string |> Ipaddr.V4.of_string_exn
            in
      ...
      Lwt.catch
        (fun () ->
          ...
          try
            ...
            >>= fun () -> Lwt.return (`OK, `Json (`O []))
          with e -> Lwt.fail e)
        (fun e ->
          let msg =
            Printf.sprintf "/privileged server err: %s" (Printexc.to_string e)
          in
  ...
  let initialize intf policy =
    create intf >>= fun t -> Lwt.return @@ start_service t policy
end
...
module Dispatcher = struct
  ...
  let dispatch t (buf, pkt) =
    ...
    if Dns_service.is_dns_query pkt then
      Log.debug (fun m -> m "Dispatcher: a dns query from %a" pp_ip src_ip)
      >>= fun () ->
      ...
      >>= fun () -> Interfaces.to_push t.interfaces nat_src_ip nat_buf
...
let create ?fifo intf_st =
      ...
      Lwt.finalize
        (fun () ->
          Lwt.catch
            ...
            (fun exn ->
              Log.err (fun m ->
                  m "intf %s err: %s" intf.Intf.dev (Printexc.to_string exn))))
        (fun () -> Log.info (fun m -> m "intf %s exited!" intf.Intf.dev))
    in
  ...
  let rec junction_lp () =
    ...
    >>= function
    | None ->
        Log.warn (fun m -> m "monitor stream closed!")
    | Some (`Up (intf, intf_starter)) ->
        if intf.Intf.dev = "eth0" then (
          ...
          >>= fun () ->
          Log.info (fun m -> m "start local service on %s..." intf.Intf.dev)
          >>= fun () -> Lwt.async service_starter ; junction_lp () )
        else if intf.dev = "eth1" then (
          let network = intf.Intf.network in
          let gw =
            network |> Ipaddr.V4.Prefix.network |> Ipaddr.V4.to_int32
            |> Int32.add Int32.one |> Ipaddr.V4.of_int32
          in
    ...
    | Some (`Down dev) ->
        Interfaces.deregister_intf interfaces dev >>= fun () -> junction_lp ()
  in
