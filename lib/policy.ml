...
type privileged =
  | SrcIP of Ipaddr.V4.t
  | DstHost of string
  | Network of Ipaddr.V4.Prefix.t
...
let get_related_peers t n =
  DomainPairSet.fold
    (fun (nx, ny) acc ->
      if nx = n then ny :: acc else if ny = n then nx :: acc else acc)
    t.pairs []
  |> Lwt.return
...
let allow_resolve t src_ip name dst_ip =
  if IpMap.mem src_ip t.resolve then
  ...
  Log.info (fun m ->
      m "allow %a to resolve %s (as %a)" pp_ip src_ip name pp_ip dst_ip)
...
let process_pair_connection t nx ny =
  if DomainPairSet.mem (nx, ny) t.pairs then Lwt.return_unit
    ...
    if
      List.exists
        (fun e -> PrivilegedSet.mem e t.privileged)
        [DstHost nx; DstHost ny]
    then
      Log.info (fun m ->
          m "Policy.connect skip privileged hostname %s|%s" nx ny)
    else
      Lwt_list.map_p Dns_service.ip_of_name [nx; ny]
      >>= fun ips ->
      let ipx = List.hd ips and ipy = List.hd @@ List.tl ips in
      Interfaces.from_same_network t.interfaces ipx ipy
      >>= function
      | false ->
          (* dns request from ipx for ny should return ipy' *)
          (* dns request from ipy for nx should return ipx' *)
          Interfaces.acquire_fake_dst t.interfaces ipx
          >>= fun ipy' ->
          Interfaces.acquire_fake_dst t.interfaces ipy
          >>= fun ipx' ->
          allow_resolve t ipx ny ipy'
          >>= fun () ->
          allow_resolve t ipy nx ipx'
          >>= fun () ->
          (* NAT: ipx -> ipy' => ipx' -> ipy *)
          (* NAT: ipy -> ipx' => ipy' -> ipx *)
          allow_transport t ipx ipy'
          >>= fun () ->
          allow_transport t ipy ipx'
          >>= fun () ->
          Nat.add_rule t.nat (ipx, ipy') (ipx', ipy)
          >>= fun () ->
          Nat.add_rule t.nat (ipy, ipx') (ipy', ipx)
          >>= fun () -> Log.info (fun m -> m "Policy.connect %s <> %s" nx ny)
      | true ->
...
let connect t nx ny =
  Lwt.async (fun () ->
      Lwt.catch
        (fun () -> process_pair_connection t nx ny)
        (function
          | Invalid_argument n ->
...
let disconnect t n ip =
  ...
  get_resolve t ip
  >>= fun resolve ->
  Lwt_list.iter_s (fun (name, _) -> forbidden_resolve t ip name) resolve
  >>= fun () ->
  (* others can't resolve n *)
  Lwt_list.iter_s
    (fun (ip, resolves) ->
      if not (List.mem_assoc n resolves) then Lwt.return_unit
      else forbidden_resolve t ip n)
    (IpMap.bindings t.resolve)
  >>= fun () ->
  get_related_peers t n
  >>= fun peers ->
  Lwt_list.iter_s (fun peer -> remove_pair t n peer) peers
  >>= fun () ->
  Nat.get_rule_handles t.nat ip
  >>= fun handles ->
  Lwt_list.iter_s
    (fun handle ->
      let translation = Nat.get_translation t.nat handle in
      let src, dst = handle in
      forbidden_transport t src dst
      >>= fun () ->
      Interfaces.release_fake_dst t.interfaces dst
      >>= fun () ->
      Nat.remove_rule t.nat handle
      >>= fun () ->
      if List.length translation <> 0 then
        ...
        >>= fun () ->
        Interfaces.release_fake_dst t.interfaces dst'
        >>= fun () -> Nat.remove_rule t.nat (src', dst')
      else Lwt.return_unit)
    handles
  >>= fun () -> Log.info (fun m -> m "Policy.disconnect %s" n)
...
let connect_for_privileged_exn t src_ip name =
  Log.info (fun m ->
      m "Policy.connect_for_privileged %a <> %s" pp_ip src_ip name)
  >>= fun () ->
  Dns_service.ip_of_name name
  >>= fun dst_ip ->
  Interfaces.from_same_network t.interfaces src_ip dst_ip
  >>= function
  | true ->
      allow_resolve t src_ip name dst_ip
      >>= fun () -> Lwt.return (Ok (src_ip, dst_ip))
  | false ->
      Interfaces.acquire_fake_dst t.interfaces src_ip
      >>= fun dst_ip' ->
      Interfaces.acquire_fake_dst t.interfaces dst_ip
      >>= fun src_ip' ->
      allow_resolve t src_ip name dst_ip'
      >>= fun () ->
      allow_transport t src_ip dst_ip'
      >>= fun () ->
      allow_transport t dst_ip src_ip'
      >>= fun () ->
      Nat.add_rule t.nat (src_ip, dst_ip') (src_ip', dst_ip)
      >>= fun () ->
      Nat.add_rule t.nat (dst_ip, src_ip') (dst_ip', src_ip)
      >>= fun () -> Lwt.return (Ok (src_ip, dst_ip'))

let connect_for_privileged t src_ip name =
  Lwt.catch
    (fun () -> connect_for_privileged_exn t src_ip name)
    (function
      | Invalid_argument n ->
          Log.err (fun m -> m "Policy.connect_for_privileged unresolvable %s" n)
          >>= fun () -> Lwt.return @@ Error src_ip
      | exn ->
          Lwt.fail exn)

let is_from_privileged_net t ip =
  PrivilegedSet.exists
    (function Network net -> Ipaddr.V4.Prefix.mem ip net | _ -> false)
    t.privileged
...
let is_authorized_resolve t ip name =
  if IpMap.mem ip t.resolve && (List.mem_assoc name @@ IpMap.find ip t.resolve)
  then Lwt.return @@ Ok (ip, List.assoc name @@ IpMap.find ip t.resolve)
  else if is_reverse_lookup name then Lwt.return @@ Error ip
  else if
    PrivilegedSet.mem (SrcIP ip) t.privileged
    || PrivilegedSet.mem (DstHost name) t.privileged
    || is_from_privileged_net t ip
  then connect_for_privileged t ip name
  ...
  List.map
    (fun (ip, resovles) ->
      let str_resolves =
        List.map
          (fun (name, resolved) ->
            Printf.sprintf "\t%s reosolved as %s" name (str_ip resolved))
          resovles
        |> String.concat "\n"
      in
      Printf.sprintf "for queries from %s:\n%s" (str_ip ip) str_resolves)
    (IpMap.bindings t.resolve)
  |> String.concat "\n"
      ...
      (fun src_ip resolvs n ->
        if 0 = Ipaddr.V4.compare src_ip new_ip then n
          ...
          if 0 = Ipaddr.V4.compare src_ip old_ip then
            IpMap.add new_ip nresolvs n
          else IpMap.add src_ip nresolvs n)
      t.resolve IpMap.empty
  in
  let () = t.resolve <- nresolv in
  Interfaces.substitute t.interfaces old_ip new_ip
  >>= fun () ->
  Nat.substitute t.nat old_ip new_ip
  >>= fun () ->
  Log.debug (fun m -> m "Policy.substitute:\n%s" (string_of_resolve t))
  >>= fun () -> Lwt.return_unit
