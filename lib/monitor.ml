...
let set_link_mtu dev mtu =
  let comm =
    ("ip", [|"ip"; "link"; "set"; "dev"; dev; "mtu"; string_of_int mtu|])
  in
  Lwt_process.exec comm
  >>= function
  | Unix.WEXITED 0 ->
      Log.info (fun m -> m "set mtu of %s to %d" dev mtu)
  | _ ->
      Log.warn (fun m -> m "set mtu of %s FAILED, continue" dev)
  ...
  let rec monitor_lp () =
    ...
    | Some l ->
        ...
        | Some (`Up (dev, cidr_addr)) ->
            Log.info (fun m -> m "link up: %s %s" dev cidr_addr)
            >>= fun () ->
            set_link_mtu dev 4000
            >>= fun () ->
            Intf.create ~dev ~cidr:cidr_addr
        ...
        | Some (`Down cidr_addr) ->
            ...
            Log.info (fun m -> m "link down: %s %s" dev cidr_addr)
            >>= fun () ->
